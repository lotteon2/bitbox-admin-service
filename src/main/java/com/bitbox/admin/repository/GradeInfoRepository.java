...
public interface GradeInfoRepository extends CrudRepository<Grade, Long> {
    ...
    @Query(value = "SELECT new com.bitbox.admin.service.response.GradeByExamIdInfoResponse(g.gradeId, g.memberInfo.memberId, g.memberInfo.memberName, e.examName, e.examId, e.perfectScore) FROM Grade g INNER JOIN Exam e ON g.exam.examId = e.examId WHERE e.examId = :examId AND e.deleted = false")
