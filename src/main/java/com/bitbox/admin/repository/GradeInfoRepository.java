...
public interface GradeInfoRepository extends CrudRepository<Grade, Long> {
    ...
    @Query(value = "SELECT new com.bitbox.admin.service.response.GradeByClassIdInfoResponse(avg(g.score), g.exam.perfectScore, g.classes.classId, e.examName) FROM Grade g INNER JOIN Exam e ON g.exam.examId = e.examId WHERE g.classes.classId = :classId AND g.exam.deleted = false AND g.memberInfo.memberAuthority = 'TRAINEE' GROUP BY g.exam.examId")
